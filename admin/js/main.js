  ...
  var retinaLogos = function () {
    ...
    if (retina) {
      if ($(".dark-theme").length > 0) {
        $('#logo_header').attr({ src: '/images/logo/logo.png' });
      } else {
        $('#logo_header').attr({ src: '/images/logo/logo.png' });
      }
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    alertSuccessElement.classList.add('show');

    setTimeout(() => {
      alertSuccessElement.classList.remove('show');
    }, 5000);
  });

  const alertDangerElement = document.getElementById('danger-alert');
  setTimeout(() => {
    alertDangerElement.classList.add('show');

    setTimeout(() => {
      alertDangerElement.classList.remove('show');
    }, 5000);
  });
});
